...
var (
	ImagePrefixes = []string{"ghcr.io/ollygarden/tulip"}
...
func Archive(dist string) config.Archive {
	return config.Archive{
		...
		Builds:       []string{dist}, // Note: Deprecated in favor of IDs, but required by current goreleaser-pro version
	}
}
...
func Package(dist string) config.NFPM {
	return config.NFPM{
		...
		License:     "Apache 2.0",
		Description: fmt.Sprintf("OllyGarden Tulip is a commercially supported distribution of the OpenTelemetry Collector - %s", dist),
		Maintainer:  "OllyGarden <support@ollygarden.com>",
...
func DockerImage(dist, arch, armVersion string) config.Docker {
	...
	return config.Docker{
		...
		Files:  []string{"config.yaml", "entrypoint.sh"},
		Goos:   "linux",
		Goarch: arch,
		Goarm:  armVersion,
	}
...
func imageName(dist string) string {
	// For Tulip, keep the distribution name as-is
	return dist
}
